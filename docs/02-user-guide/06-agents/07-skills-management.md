...
同一个任务中，智能体会综合多个来源的 Skill：

| 来源 | 典型路径 | 适用范围 |
|------|----------|----------|
| 项目级 Skill | `<workdir>/.agents/skills/` | 当前项目或工作目录 |
| Claude 兼容 Skill | `<workdir>/.claude/skills/` | 兼容已有 Claude 项目配置 |
| 智能体 Skill | `agents/<agent_id>/skills/` | 当前智能体 |
| 全局 Skill | 全局技能目录 | 所有智能体 |

当 `.agents/skills` 和 `.claude/skills` 同时存在时，DesireCore 优先使用 `.agents/skills`。这允许你在不破坏旧项目的前提下逐步迁移到 DesireCore 原生结构。

## Skill 导出

在 Skill 详情页可以单独导出 Skill。导出的 ZIP 会包含：

- `SKILL.md`
- 可选的脚本、参考资料、模板和资源文件
- 元数据、输入输出 schema 和本地化内容

导出 Skill 适合在团队间共享单个能力，而不是导出整个智能体。

## 多语言和资源引用

Skill 可以声明本地化内容。界面语言为中文时，智能体会优先加载中文说明；界面语言为英文时，会加载英文说明。这样同一个 Skill 可以在多语言团队里保持一致行为。

Skill 文档中可以使用 `${SKILL_DIR}` 或 `{{SKILL_DIR}}` 引用自身目录下的资源，例如模板、脚本或示例文件。执行时系统会把占位符解析为真实路径，避免在不同设备上硬编码绝对路径。

:::info 自动注入
并非所有 Skill 都会自动进入模型上下文。只有被显式调用、被智能体判定需要，或在元数据中允许自动注入的 Skill，才会被加载。这样可以减少上下文污染和无关工具暴露。
:::

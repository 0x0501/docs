...
从市场或远程仓库安装的智能体可以检查更新。当远端有新版本时，DesireCore 会比较本地 Git 状态：

- 本地没有改动：可以直接更新
- 本地有改动但不冲突：自动合并并记录 commit
- 本地和远端修改了同一文件：进入冲突解决流程

冲突出现时，界面会显示冲突 badge，并打开解决对话框。你可以：

| 操作 | 说明 |
|------|------|
| 保留本地 | 使用你当前的文件内容 |
| 使用远端 | 使用市场或远程仓库的新版本 |
| 手动编辑 | 在 DiffEditor 中合并内容后保存 |
| 保留两者 | 另存一份，避免丢失任一版本 |

冲突解决完成后，系统会自动提交一次合并结果。若远端文件被删除，本地修改也会在冲突列表中明确展示。

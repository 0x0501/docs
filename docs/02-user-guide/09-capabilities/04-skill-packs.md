...
| 方式 | 说明 | 适合场景 |
|------|------|----------|
| 显式调用 | 用户通过 `/skill:技能名`、技能列表或界面指定某个技能 | 你明确知道要用哪个技能 |
| 自动匹配 | 智能体根据任务和技能描述判断是否加载 | 你只描述目标，不关心具体技能 |
| 项目注入 | 当前工作目录的 `.agents/skills` 或 `.claude/skills` 提供项目级能力 | 团队规范、项目脚本和模板随项目维护 |
| 自动注入 | Skill 元数据允许时，系统可把内容自动加入上下文 | 轻量规则、常驻约束或明确声明的能力 |
...
自动注入会受 Skill 配置控制。对于大型 Skill，系统通常只注入摘要或在需要时再加载详情，以节省上下文。
...
项目级技能放在工作目录中，跟随项目而不是跟随某个智能体。推荐路径是：

```
<project>/.agents/skills/<skill_id>/SKILL.md
```

DesireCore 也兼容：

```
<project>/.claude/skills/<skill_id>/SKILL.md
```

项目级技能适合放团队代码规范、项目部署步骤、报告模板、数据处理脚本等内容。
...
```md
使用 `${SKILL_DIR}/templates/report.docx` 作为报告模板。
```

运行时 `${SKILL_DIR}` 会被替换为该 Skill 的真实路径。
